...
const StudyBook: React.FC<StudyBookProps> = ({ language }) => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setContent('');
    try {
      const generatedContent = await generateStudyMaterial(language, topic, level);
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="mb-6">
        <h2 className="text-2xl font-bold text-slate-800">Study Book</h2>
        <p className="text-sm text-slate-500">Generate personalized study materials for any topic.</p>
      </header>
      <div className="bg-white rounded-xl shadow-md p-6 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder={`e.g., "How to order coffee in ${language.name}"`}
        ...
        >
         ...
         <div className="mt-4">
             <h4 className="text-sm font-medium text-slate-600 mb-2">Or try a common topic:</h4>
      ...
      <div className="flex-1 bg-white rounded-xl shadow-md p-6 overflow-y-auto">
        ...
        {!isLoading && !content && !error && (
            <div className="text-center text-slate-500 h-full flex flex-col justify-center items-center">
                <Icon name="study" className="w-16 h-16 text-slate-300 mb-4"/>
                <h3 className="text-lg font-semibold">Your study guide will appear here.</h3>
